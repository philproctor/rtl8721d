...
static TASK_QUEUE: StaticMutex<Channel<Pin<Box<dyn Future<Output = ()> + Send + Sync>>>> =
...
impl Executor {
    ...
    pub fn spawn(f: impl Future<Output = ()> + Send + Sync + 'static) {
    ...
    pub fn run() {
        spin_on::spin_on(async {
            let mut tasks: VecDeque<Pin<Box<dyn Future<Output = ()> + Send + Sync>>> =
                Default::default();
            loop {
